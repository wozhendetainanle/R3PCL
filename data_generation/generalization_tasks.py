...
def task_shape(condition='xysc'): 
    ...
    s_odd = Shape()
    s_odd2 = Shape()
    shape = [[s.clone()] for i in range(n_samples-2)] + [[s_odd]] + [[s_odd2]]
...
def task_pos(condition='xycsid'):
    ...
    if 'xy' in condition:
        ...
        xy_odd2 = np.random.rand(2) * (1-size_max) + size_max/2
    elif 'x' in condition:
        xy_odd = np.random.rand(2) * (1-size_max) + size_max/2
        xy_odd2 = np.random.rand(2) * (1-size_max) + size_max/2
        xy_odd[0] = xy[0]
    ...
    while np.linalg.norm(xy_odd2 - xy) < 0.2:
        # xy_odd = np.random.rand(2)
...
def task_size(condition='xyidc'):
    ...
    prop2 = np.random.rand() * (max_prop - min_prop) + min_prop
    ...
    size_odd = size * prop
    size_odd2 = size * prop2
    if size + size_odd > 0.9:
        size, size_odd = size/(size + size_odd)*0.9, size_odd/(size + size_odd)*0.9
    
    if size + size_odd2 > 0.9:
        size, size_odd2 = size/(size + size_odd2)*0.9, size_odd2/(size + size_odd2)*0.9
...
def task_color(condition='xysid'):
    ...
    h_odd2 = (h + (np.random.rand() * (1-min_diff*2) + min_diff) * np.random.choice([-1,1]))%1

    color = [[[h*0.95+0.05*np.random.rand(),s*0.5+np.random.rand()*0.5,v]] for i in range(n_samples-2)] + [[[h_odd, s, v]]] + [[[h_odd2, s, v]]]
...
def task_inside(condition='c'):
    ...
    xy1_odd = xy1[-2:-1]
    xy2_odd2 = np.random.rand(100,2) * range_2 + starting_2 
    xy1_odd2 = xy1[-1:]

    xy2_odd = xy2_odd[(np.abs(xy2_odd - xy1_odd) > (size_a[-1] + size_b[-1])/2).any(1)]
    xy2_odd = xy2_odd[0:1]
    xy2_odd2 = xy2_odd2[(np.abs(xy2_odd2 - xy1_odd2) > (size_a[-1] + size_b[-1])/2).any(1)]
    xy2_odd2 = xy2_odd2[0:1]
    ...
    s2 = Shape(gap_max=0.01)
    shapes.append([s1,s2])
    xy2 = np.concatenate([np.array(xy2)*size_a[:-2,None] + xy1[:-2],xy2_odd], 0)
    xy2 = np.concatenate([xy2,xy2_odd2], 0)
...
def task_count(condition='xysidc', condition_arr='xysid'):
    ...
    n_objects_odd = [n for n in np.random.randint(3,10, size=20) if n!=n_objects][0]
    n_objects_odd2 = [n for n in np.random.randint(3,10, size=20) if n!=n_objects][0]

    n_objs = np.ones(n_samples, dtype=int)*n_objects
    n_objs[-2] = n_objects_odd
    n_objs[-1] = n_objects_odd2


    max_n_objs = max(n_objects, n_objects_odd)
    max_n_objs = max(max_n_objs, n_objects_odd2)
    ...
    shapes = [[all_shapes[k].clone() for k in id_idx[i][:n_objects].astype(int)] for i in range(n_samples-2)]
    shapes = shapes + [[all_shapes[k].clone() for k in id_idx[-1][:n_objects_odd].astype(int)]] + [[all_shapes[k].clone() for k in id_idx[-1][:n_objects_odd2].astype(int)]]
...
def task_pos_pos_2(condition='xycsid'):
    ...
    # unique_sc
    sc = np.stack([unique_sc[:-2], np.concatenate([unique_sc[:-4], unique_sc[-2:]],0)], 1)
...
def task_pos_pos_4(condition='cid'):
    ...
    n_objects = np.random.randint(3, 4)
    ...
    # get the highest ys
    y_reg = np.insert(y_all[:-2], index, y_max[:-2], axis=1)
    # sample  index other than index
    index_odd = [i for i in np.random.randint(n_objects, size=10) if i!=index][0] 
    y_odd = np.insert(y_all[-2], index_odd, y_max[-2])

    index_odd2 = [i for i in np.random.randint(n_objects, size=10) if i!=index][0] 
    y_odd2 = np.insert(y_all[-2], index_odd2, y_max[-2])
    
    y = np.concatenate([y_reg, y_odd[None,:]], 0)
    y = np.concatenate([y, y_odd2[None,:]], 0)
...
def task_pos_size_2(condition='cid'):
    ...
    max_size_sc = 0.9*4/5 # n_forms
    min_size_sc = max_size_sc*1/3
    max_r, min_r = 2/3, 1/2
    r = np.random.rand(n_samples) * (max_r-min_r) + min_r
    ...
    if task in ['=', '!=']:
        ...
        if odd_condition == 0:
            if task == '=':
                ...
                co2[-2] = sample_over_range_t(1, np.array([0,1]), size[-2:-1])
            else:
                co2[:-2] = np.random.rand(n_samples-2, 2) * (1-size[:-2]) + size[:-2]/2
...
def task_pos_shape_2(condition='cs'):
    ...
    shapes_odd = [shapes[idx].clone() for idx in shape_idx]
    all_shapes.append(shapes_odd)
    shape_idx = odd_pairs2[perm].flatten()
    shapes_odd2 = [shapes[idx] for idx in shape_idx]
    all_shapes.append(shapes_odd2)
...
def task_pos_rot_1(condition='xycs'):
    ...
    angles_odd2 = np.random.rand(n_objects) * 2 * np.pi

    angles = np.ones([n_samples, n_objects]) * angles[None,:]
    angles[-1] = angles_odd
    angles[-2] = angles_odd2
...
def task_pos_flip_1(condition='sc'):
    ...
    odd_flips2 = np.random.randint(2, size=n_objects-1)
    # odd_flips = np.random.randint(2, size=n_objects)

    while (flips==odd_flips).all():
        odd_flips = np.random.randint(2, size=n_objects-1)
    
    while (flips==odd_flips2).all():
        odd_flips2 = np.random.randint(2, size=n_objects-1)

    flips = np.stack([flips]*(n_samples-2) + [odd_flips] + [odd_flips2], 0)
...
def task_pos_flip_2(condition='csid'):
    ...
    for i in range(n_samples):
        ...
        if odd_condition == 1 and i == n_samples-1:
            ...
            s1.rotate(a_)
        elif odd_condition == 1 and i == n_samples-2:
            a_ = a_ + np.random.choice([-1,1]) * np.pi/4
            s1.rotate(a_)
        else:
            s1.rotate(a_)

        if not (odd_condition == 0 and ((i == n_samples-1) or (i == n_samples-2))):
            s2.flip()
...
def task_pos_col_2(condition='xychsid'):
    ...
    color_idx_odd2 = color_idx[np.random.permutation(len(color_idx))]
    while (color_idx == color_idx_odd).all():
        color_idx_odd = color_idx[np.random.permutation(len(color_idx))]
    while (color_idx == color_idx_odd2).all():
        color_idx_odd2 = color_idx[np.random.permutation(len(color_idx))]

    if 'h' in condition:
        h_unique = np.arange(unique_colors) / unique_colors
        h_unique2 = h_unique + np.random.rand()
...
def task_pos_inside_3(condition='c'):
    ...
    n_objects_samples = np.random.randint(low=4, high=6, size=n_samples)
...
def task_pos_contact(condition='c'):
    ...
    angle3 = angle1 + np.pi / 2 * np.random.choice([-1, 1])
...
def task_pos_count_2(condition='cid'):
    ...
    n_objects_aligned_v = np.random.choice(np.arange(5,10), size=3, replace=False)

    n_objects_aligned = np.ones(n_samples) * n_objects_aligned_v[0]
    n_objects_aligned[-1] = n_objects_aligned_v[1]
    if random.random() > 0.5:
        n_objects_aligned[-2] = n_objects_aligned_v[2]
    else:
        n_objects_aligned[-2] = n_objects_aligned_v[1]
...
def task_pos_count_3(condition='cid'):
    ...
    n_lines_v = np.random.choice(np.arange(6,11), size=3, replace=False)

    n_lines = np.ones(n_samples) * n_lines_v[0]
    n_lines[-1] = n_lines_v[1]
    if random.random() > 0.5:
        n_lines[-2] = n_lines_v[2]
    else:
        n_lines[-2] = n_lines_v[1]
    n_lines = n_lines.astype(int)
    
    n_lines_objects = np.random.randint(2,6, size=[n_samples, n_lines_v.max()])
...
def task_pos_count_4(condition='cid'):
    ...
    n_objects_3[-2] = max(n_objects_3[-2] + np.random.choice([-1, 1])*np.random.randint(1,3),1)
...
def task_size_size_2(condition='xycid'):
    ...
    size_1 = np.random.rand(n_samples-2) * (max_size - min_size) + min_size
    r_prop = np.random.rand(n_samples-2) * (2/3 - 1/3) + 1/3
    idx = np.random.choice(np.arange(n_samples-2), size=np.random.randint(2,n_samples-2), replace=False)
    ...
    odd_condition = np.random.randint(2)
    odd_condition2 = np.random.randint(2)
    ...
    if odd_condition2 == 0:
        # odd_n_unique_sizes = 1
        size_2 = np.random.rand() * (max_size - min_size) + min_size
        if size_2*3 >0.9:
            size_2 = size_2*0.9/3
        size_2 = np.ones([3])*size_2
    else: 
        # odd_n_unique_sizes = 3 
        size_2 = np.random.rand() * (max_size - min_size) + min_size
        r_prop = np.random.rand(2) * (2/3 - 1/3) + 1/3
        size_2 = np.array([size_2, size_2*r_prop[0], size_2*r_prop[0]*r_prop[1]])
        # (s + s*r_prop[0] + s*r_prop[0]*r_prop[1])*r < 0.8
        r = 0.9 / size_2.sum()
        size_2 = size_2 * r


    size = np.concatenate([size_reg, size_[None,:], size_2[None,:]], axis=0)
...
def task_size_size_3(condition='xycid'):
    ...
    # odd_n_unique_sizes = 3 
    size_ = np.random.rand(n_samples-2) * (max_size - min_size) + min_size
    r_prop = np.random.rand(n_samples-2, 2) * (2/3 - 1/3) + 1/3
    ...
    if odd_condition2==0:
        ...
        size_2 = np.array([size_1, size_1, size_2])
...
def task_size_size_4(condition='xycid'):
    ...
    prop_odd = np.random.choice([5/6, 4/5, 1/3.1, 1/4])
    prop_odd2 = np.random.choice([5/6, 4/5, 1/3.1, 1/4])

    size[-1,1] = size[-1,0] * prop_odd
    size[-2,1] = size[-2,0] * prop_odd2
...
def task_size_size_5(condition='cid'):
    ...
    shuf_perm_odd2, _ = sample_shuffle_unshuffle_indices(n_objects)
    while np.abs(shuf_perm_odd2-shuf_perm).sum()==0:
        shuf_perm_odd2, _ = sample_shuffle_unshuffle_indices(n_objects)

    size2 = size.copy()
    
    perms1 = np.stack([shuf_perm1]*(n_samples), 0)
    perms2 = np.stack([shuf_perm]*(n_samples-2)+ [shuf_perm_odd] + [shuf_perm_odd2], 0)
...
def task_size_shape_1(condition='xyscid'):
    ...
    if np.random.randint(2)==0:
        ...
        size[-2] = np.array([size[-2,0], size[-2,2], size[-2,1], size[-2,3]])
    else:
...
def task_size_shape_2(condition='xyc', reference='id'):
    ...
    size_switch = (size[-2,1], size[-2,3])
    size[-2:-1,1] = size_switch[1]
    size[-2:-1,3] = size_switch[0]
...
def task_size_color_2(condition='idv'):
    ...
    color_idx[-2] = np.roll(color_idx[-2], 1).tolist()
...
def task_size_contact(condition='sc'):
    ...
    cond_reg, cond_odd, cond_odd2 = np.random.choice(all_conditions, replace=False, size=3)
    if random.random() > 0.5:
        cond_odd2 = cond_odd
    ...
    n_uniques_sizes_odd2 = n_unique_sizes[cond_odd2]
    ...
    if n_uniques_sizes_odd2>1:
        # max_r, min_r = 2/3, 1/5
        min_diff = (max_r - min_r)/4
        if n_uniques_sizes_odd2>2:
            r = sample_over_range_t(1, np.array([min_r - min_diff/2, max_r + min_diff/2]), np.ones([1, n_uniques_sizes_odd2-1])* min_diff)
        else:
            r = np.random.rand(1) * (max_r-min_r) + min_r
        u_size = np.random.rand(1) * (max_size - min_size) + min_size

        u_size_odd2 = np.concatenate([u_size[:,None]*r, u_size[:,None]], 1)
    else:
        u_size_odd2 = np.random.rand(1) * (max_size - min_size) + min_size
        u_size_odd2 = np.ones([1,1]) * u_size_odd2
    
    perm_reg = perm[cond_reg]
    perm_odd = perm[cond_odd]
    perm_odd2 = perm[cond_odd2]

    size = [u_size_reg[i, perm_reg] for i in range(n_samples-2)]
    size += [u_size_odd[0, perm_odd]]
    size += [u_size_odd2[0, perm_odd2]]
...
def task_size_count_1(condition='idc'):
    ...
    n_objects_samples_odd2 = sample_int_sum_n(n_groups, n_objects, min_v=1).astype(int)
    while (n_objects_samples_odd2 == n_objects_samples).all():
        n_objects_samples_odd2 = sample_int_sum_n(n_groups, n_objects, min_v=1).astype(int)
    ...
    size_idx_odd2 = [[i]*n_objects_samples_odd2[i] for i in range(n_groups)]
    size_idx_odd2 = np.array(cat_lists(size_idx_odd2)) 
    ...
    min_diff = (max_r - min_r)/2
    r = sample_over_range_t(1, np.array([min_r - min_diff/2, max_r + min_diff/2]), np.ones([1, n_unique_sizes])* min_diff)
    u_size = np.random.rand(n_samples-2) * (max_size - min_size) + min_size
    u_size = np.concatenate([u_size[:,None]*r, u_size[:,None]], 1)

    size = [u_size[i, size_idx] for i in range(n_samples-2)] + [u_size[-1, size_idx_odd]] + [u_size[-1, size_idx_odd2]]
    size = np.stack(size, 0)
...
def task_size_count_2(condition='c'):
    ...
    n_groups = np.random.choice([2,3,4], size=3, replace=False)
    # n_unique_sizes = n_groups
    ...
    prb = random.random()
    if prb > 0.5:
        n_objects_samples_odd2 = sample_int_sum_n(n_groups[1], n_objects, min_v=1)
        size_idx_odd2 = [[i]*n_objects_samples_odd2[i] for i in range(n_groups[1])]
    else:
        n_objects_samples_odd2 = sample_int_sum_n(n_groups[2], n_objects, min_v=1)
        size_idx_odd2 = [[i]*n_objects_samples_odd2[i] for i in range(n_groups[2])]
    ...
    if prb < 0.5:
        r = sample_over_range_t(1, np.array([min_r - min_diff/2, max_r + min_diff/2]), np.ones([1, n_groups[2]])* min_diff)
    u_size_odd2 = np.random.rand(1) * (max_size - min_size) + min_size
    u_size_odd2 = np.concatenate([u_size_odd2[:,None]*r, u_size_odd2[:,None]], 1)

    size = [u_size[i, size_idx] for i in range(n_samples-2)] + [u_size_odd[0, size_idx_odd]] + [u_size_odd2[0, size_idx_odd2]]
    size = np.stack(size, 0)
...
def task_shape_shape(condition='xyscid'):
    ...
    if 'id' in condition:
        ...
        s = Shape()
        shape = [[s.clone(), s.clone()] for _ in range(n_samples-2)] + [[s.clone(), Shape()]] + [[s.clone(), Shape()]]
...
def task_shape_color_2(condition='idcs'):
    ...
    color_idx[-2] = np.roll(color_idx[-2], 1)
...
def task_shape_color_3(condition='idcs'):
    ...
    color_idx[-2][1] = 0
    color_idx[-2][n_objects_samples[-2][0]] = 1
...
def task_shape_inside(condition='c'):
    ...
    for _ in range(max_attempts):
        ...
        samples_i = sample_position_inside_1(s2, s1, size_b[-2]/size_a[-2])
        samples.append(samples_i)
...
def task_shape_contact_2(condition='s'):
    ...
    shape[-2] = [shape[-2][0], shape[-2][2], shape[-2][1]]
    xy = []
...
def task_shape_count_1(condition='idcs'):
    ...
    n_objects_samples_odd2 = sample_int_sum_n(n_groups, n_objects, min_v=1)
    while (np.sort(n_objects_samples_odd2) == np.sort(n_objects_samples)).all():
        n_objects_samples_odd2 = sample_int_sum_n(n_groups, n_objects, min_v=1)
    ...
    shape_idx_odd2 = [[i]*n_objects_samples_odd2[i] for i in range(n_groups)]
    shape_idx_odd2 = np.array(cat_lists(shape_idx_odd2)) 
    ...
    if 'id' in condition:
        ...
        for i in range(n_samples):
            ...
            if i == n_samples-1:
                ...
                shape.append([unique_shape[idx].clone() for idx in shape_idx_odd2])
            else:
...
def task_shape_count_2(condition='idcs'):
    ...
    n_groups = np.random.choice([2,3,4], replace=False, size=3)

    n_objects = np.random.randint(n_groups.max(), n_groups.max()*2)

    n_groups_odd = n_groups[1]
    if random.random() > 0.5:
        n_groups_odd2 = n_groups[1]
    else:
        n_groups_odd2 = n_groups[2]
    n_groups = n_groups[0]
    ...
    for i in range(n_samples):
        n_objects_samples.append(sample_int_sum_n(n_groups, n_objects, min_v=1))
    n_objects_samples.append(sample_int_sum_n(n_groups_odd2, n_objects, min_v=1))    
    n_objects_samples.append(sample_int_sum_n(n_groups_odd, n_objects, min_v=1))

    shape_idx = [cat_lists([[j]*n_objects_samples[i][j] for j in range(n_groups)]) for i in range(n_samples-2)]
    shape_idx += [cat_lists([[j]*n_objects_samples[-2][j] for j in range(n_groups_odd2)])]
    ...
    if 'id' in condition:
        ...
        for i in range(n_samples):
            if i == n_samples-1:
                ...
                unique_shape = [Shape() for j in range(n_groups_odd2)]
            else:
                unique_shape = [Shape() for j in range(n_groups)]
        ...
        shape = []
        greater_odd = max(n_groups_odd, n_groups_odd2)
        unique_shape = [Shape() for j in range(max(n_groups,greater_odd))]
        for i in range(n_samples):
            shape.append([unique_shape[idx].clone() for idx in shape_idx[i]])
...
def task_rot_rot_3(condition='xyidsc'):
    ...
    angle_diff_odd2 = np.mod(angle_diff + angle_diff_diff * np.random.choice([-1,1]), np.pi)
    angle_diffs = np.ones(n_samples) * angle_diff
    angle_diffs[-1] = angle_diff_odd
    angle_diffs[-2] = angle_diff_odd2
...
def task_rot_flip_1(condition='cs'):
...
def task_rot_color(condition='idcsv'):
    ...
    while True:
        color_idx_copy = copy.deepcopy(color_idx[-1])
        np.random.shuffle(color_idx[-1])
        if not np.array_equal(color_idx[-1], color_idx_copy):
            break
    
    while True:
        color_idx_copy = copy.deepcopy(color_idx[-2])
        np.random.shuffle(color_idx[-2])
        if not np.array_equal(color_idx[-2], color_idx_copy):
            break
...
def task_rot_inside_1(condition='xysc'):
    ...
    angle_odd2 = angles[-2] + (np.random.rand() * (1-0.5) + 0.5) * np.pi
    ...
    for i in range(n_samples):
        ...
        if done1:
            ...
            if i == n_samples-1:
                ...
                if odd_cond == 0:
                    s2.rotate(angles[i])
                    s4.rotate(angle_odd2)
                else:
                    s2.rotate(angle_odd2)
                    s4.rotate(angles[i])
            else:
                s2.rotate(angles[i])
                s4.rotate(angles[i])
...
def task_color_color_1(condition='xysid_hs-v'):
    ...
    if variable == 'h':
        ...
        h[-2, 1] = (h[-2, 1] + (np.random.rand() * (1-min_diff*2) + min_diff) * np.random.choice([-2,1]))%1
    elif 'h' in condition:
        h = np.random.rand(n_samples, 2)
    else:
        h = np.random.rand()
    ...
    if variable == 's':
        ...
        s[-2, 1] = (s[-2, 1] + (np.random.rand() * (1-min_diff*2) + min_diff + 0.2) * np.random.choice([-2,1]))%1
    elif 's-' in condition:
    ...
    if variable == 'v':
        ...
        v[-2, 1] = (v[-2, 1] + (np.random.rand() * (0.5-min_diff*2) + min_diff + 0.5) * np.random.choice([-2,1]))%1
    elif 'v' in condition:
        v = (np.random.rand(n_samples, 2) * 0.5 + 0.35)
    else:
        v = np.random.rand() * 0.5 + 0.35
...
def task_color_color_2(condition='nxysid_hs-v'):
    ...
    if variable == 'h':
        ...
        n_objects_odd2 = np.random.randint(1, n_objects[-1])
        min_diff = 0.15
        ...
        h[-2, :n_objects_odd2] = (h[-1, :n_objects_odd2] + (np.random.rand(n_objects_odd2) * (1 - min_diff*2) + min_diff) * np.random.choice([-1,1], size=n_objects_odd2))%1
    ...
    if variable == 's':
        ...
        s[-2, :n_objects_odd2] = (s[-1, :n_objects_odd2] + (np.random.rand(n_objects_odd2) * (0.7-min_diff*2) + min_diff + 0.3) * np.random.choice([-1,1], size=n_objects_odd2))%1
    ...
    if variable == 'v':
        ...
        v[-2, :n_objects_odd2] = (v[-1, :n_objects_odd2] + (np.random.rand(n_objects_odd2) * (0.5-min_diff*2) + min_diff + 0.5) * np.random.choice([-1,1], size=n_objects_odd2))%1
...
def task_color_inside_2(condition='xysid_hs-v'):
    ...
    color[-3, n_objects:] = color[-1, n_objects:]
    color = color[:-1]
...
def task_color_contact(condition='sids-v'):
    ...
    n_uniques_colors_odd2 = n_unique_colors[cond_odd2]
    ...
    if n_uniques_colors_odd2>1:
        min_diff = 0.15
        u_h_odd2 = sample_over_range_t(1, np.array([0,1]), min_diff*np.ones([1, n_uniques_colors_odd2]))
    else:
        u_h_odd2 = np.random.rand(1) * (max_size - min_size) + min_size
        u_h_odd2 = np.ones([1,1]) * u_h_odd2

    perm_reg = perm[cond_reg]
    perm_odd = perm[cond_odd]
    perm_odd2 = perm[cond_odd2]

    h = [u_h_reg[i, perm_reg] for i in range(n_samples-2)]
    h += [u_h_odd[0, perm_odd]]
    h += [u_h_odd2[0, perm_odd2]]
    h = np.stack(h, 0)
...
def task_color_count_1(condition='csids-v'):
    ...
    while (n_objects_samples_odd == n_objects_samples).all() or (n_objects_samples_odd[::-1] == n_objects_samples).all():
        n_objects_samples_odd = sample_int_sum_n(n_groups, n_objects, min_v=1)
    
    n_objects_samples_odd2 = sample_int_sum_n(n_groups, n_objects, min_v=1)
    while (n_objects_samples_odd2 == n_objects_samples).all() or (n_objects_samples_odd2[::-1] == n_objects_samples).all():
        n_objects_samples_odd2 = sample_int_sum_n(n_groups, n_objects, min_v=1)
    ...
    color_idx_odd2 = [[i]*n_objects_samples_odd2[i] for i in range(n_groups)]
    color_idx_odd2 = np.array(cat_lists(color_idx_odd2)) 
    ...
    color = [color_u[i,color_idx] for i in range(n_samples-2)] + [color_u[-2,color_idx_odd]] + [color_u[-1,color_idx_odd2]]
...
def task_color_count_2(condition='csids-v'):
    ...
    n_groups = np.random.choice([2,3,4,5], replace=False, size=3)
    ...
    color_idx = [cat_lists([[j]*n_objects_samples[i][j] for j in range(n_groups)]) for i in range(n_samples-2)]
    color_idx += [cat_lists([[j]*n_objects_samples[-2][j] for j in range(n_groups_odd)])]
    color_idx += [cat_lists([[j]*n_objects_samples[-1][j] for j in range(n_groups_odd2)])]
    # shape_idx = cat_lists(shape_idx) 
    ...
    h_u = sample_over_range_t(n_samples-2, np.array([0,1]), np.ones([n_samples-2, n_groups])*min_diff)
    h_u_odd = sample_over_range_t(1, np.array([0,1]), np.ones([1, n_groups_odd])*min_diff)
    h_u_odd2 = sample_over_range_t(1, np.array([0,1]), np.ones([1, n_groups_odd2])*min_diff)

    if 's-' in condition:
        max_odd = max(n_groups_odd2, n_groups_odd)
        s = (np.random.rand(n_samples, max(n_groups, max_odd)) * 0.4 + 0.6)
    else:
        s = np.random.rand() * 0.4 + 0.6
        max_odd = max(n_groups_odd2, n_groups_odd)
        s = s * np.ones([n_samples, max(n_groups, max_odd)])
        
    if 'v' in condition:
        max_odd = max(n_groups_odd2, n_groups_odd)
        v = (np.random.rand(n_samples, max(n_groups, max_odd)) * 0.5 + 0.5)
    else:
        v = np.random.rand() * 0.5 + 0.5
        max_odd = max(n_groups_odd2, n_groups_odd)
        v = v * np.ones([n_samples, max(n_groups, max_odd)])

    color_u = np.stack([h_u,s[:n_samples-2, :n_groups],v[:n_samples-2, :n_groups]], 2)
    color_u_odd = np.stack([h_u_odd,s[-1:, :n_groups_odd],v[-1:, :n_groups_odd]], 2)
    color_u_odd2 = np.stack([h_u_odd2,s[-1:, :n_groups_odd2],v[-1:, :n_groups_odd2]], 2)

    color = [color_u[i, color_idx[i]] for i in range(n_samples-2)] + [color_u_odd[0, color_idx[-2]]] + [color_u_odd2[0, color_idx[-1]]]
...
def task_inside_inside_1(condition='c'):
    ...
    sampled_conditions[-2] = np.random.choice(conditions_o) 
...
def task_inside_inside_2(condition='c'):
    ...
    n_objects_out[-2] = np.random.randint(2, n_objects[-2])
...
def task_inside_count_1(condition='c'):
    ...
    n_objects_inside[-2] = n_objects_inside[-2] + np.random.choice([-2,1]) 
...
def task_inside_contact(condition='sc'):
    ...
    cond = [cond]*(n_samples-2) + [1-cond] + [1-cond]
    ...
    for i in range(n_samples):
        ...
        for j in range(n_objects):
            ...
            if j<n_objects_in:
                ...
                if i != n_samples-1 and i != n_samples-2:
                    so2.flip()
                    si2.flip()
...
def task_contact_contact_1(condition='cid'):
    ...
    n_obj_connected[-2] = np.random.randint(n_obj_connected[-2])
...
def task_contact_count_1(condition='csid'):
    ...
    contact_idx_odd2 = [[i]*n_objects_samples_odd2[i] for i in range(n_groups)]
    contact_idx_odd2 = np.array(cat_lists(contact_idx_odd2))
    ...
    for i in range(n_samples):
        ...
        if i == n_samples-2:
            n_obj_s = n_objects_samples_odd2
        xy_ = []
        n = 0
        positions_ = [] 
        clump_size_ = []
...
def task_contact_count_2(condition='csid'):
    ...
    for i in range(n_samples):
        n_obj_s = n_objects_samples[i]
        n_g = n_groups_odd if i == n_samples-2 else n_groups
        if i == n_samples-1:
            n_g = n_groups_odd2
        xy_ = []
        n = 0
...
def task_count_count(condition='xyscid'):
    ...
    n_samples = 5
    n_objects = np.random.choice(np.arange(3, 8), size=[n_samples], replace=False)*2
    if np.random.randint(2)==0:
        n_objects[:-2] = n_objects[:-2] + np.random.choice([-1,1])
    else:
...
def task_sym_mir(condition='cid'):
    ...
    if odd_condition == 1:
        idx = np.random.randint(2)
        size[-1,idx] = size[-1,idx] * (np.random.rand()* (2/3 - 1/3) + 1/3)
        size[-2,idx] = size[-2,idx] * (np.random.rand()* (2/3 - 1/3) + 1/3)
    ...
    if 'id' in condition:
        ...
        for i in range(n_samples):
            ...
            if odd_condition == 4 and ((i == n_samples-1) or (i == n_samples-2)):
                a_ = a_ + np.random.choice([-1,1]) * (np.random.rand()* (1-1/2)+1/2)*np.pi
            
            s1.rotate(a_)

            if not (odd_condition == 2 and ((i == n_samples-1) or (i == n_samples-2))):
                s2.flip()
            
            s2.rotate(a_)

            if odd_condition == 0 and ((i == n_samples-1) or (i == n_samples-2)):
                s_ = Shape()
                if np.random.rand()>0.5:
                    s1 = s_
                else:
...
def task_sym_rot(condition='cid'):
    ...
    if 'id' in condition:
        ...
        for i in range(n_samples):
            ...
            if odd_condition == 3 and ((i == n_samples-1) or (i == n_samples-2)):
...
def task_size_sym_1(condition='cid'):
    ...
    if odd_condition2 == 0:
        x1[-2] = np.random.rand(n_objects) * (0.5 - size[-2]) + size[-2]/2
    ...
    if 'id' in condition:
        ...
        for i in range(n_samples):
            ...
            if odd_condition2 in [1,2] and i==n_samples-2:
                odd_samples2 = np.random.choice(n_objects, size=np.random.randint(1,n_objects-1), replace=False)
            ...
            for j in range(n_objects):
                ...
                elif not (odd_condition2 == 2 and i == n_samples-2 and j in odd_samples2):
                    s2.flip()
                ...
                if odd_condition2 == 1 and i == n_samples-2 and j in odd_samples2:
                    s_ = Shape()
                    if np.random.rand()>0.5:
                        s1 = s_
                    else:
...
def task_size_sym_2(condition='cid'):
    ...
    for i in range(n_samples):
        ...
        if i == n_samples-1 and odd_condition==0:
            ...
            rad = np.concatenate([rad]*2, 1)
        elif i == n_samples-2 and odd_condition2==0:
            rad = np.random.rand(n_samples_over, n_objects) * (0.5 - shuf_size[i:i+1,:n_objects]*np.sqrt(2)) + shuf_size[i:i+1,:n_objects]*np.sqrt(2)/2
            rad = np.concatenate([rad]*2, 1)
        else:
            margin_r = 1.2
    ...
    if 'id' in condition:
        ...
        for i in range(n_samples):
            ...
            if odd_condition2 in [1,2] and i==n_samples-2:
                odd_samples2 = np.random.choice(n_objects, size=np.random.randint(1,n_objects), replace=False)
...
def task_sym_sym_1(condition='cid'):
    ...
    if odd_condition == 5:
        ...
        a_odd2 = (np.random.rand(100) * 1 - 1/2) * np.pi
        a_odd2 = a_odd2[np.abs(a[0] - a_odd2)>np.pi/3][0]
...
def task_sym_sym_2(condition='cid'):
    ...
    if odd_condition == 4:
        ...
        diffs = sample_over_range(np.array([min_diff/2, 1+min_diff/2]), np.ones(3)*min_diff) * np.pi
